...
Twitch в месяц посещают 214,4 миллионов пользователей (включая случайных)[[1]](https://www.similarweb.com/ru/website/twitch.tv/)
...
подписываются на сообщения внутри стрима, что будет реализовано через Pub/Sub [[5]](https://redis.io/docs/manual/pubsub/) в Redis, который уже используется.
...
Итого (без учета будущего количества реплик)

```
 0,3248 Терабайт +  10,4 Мегабайт +  0,0336 Терабайт + 56,16 Мегабайт ~ 0,4 Терабайт
```
...
Записи, как уже отмечалось ранее, происходят намного реже,чем чтение, соответсвенно следует использовать реплики на чтение данных из PostgreSQL. 
Следует использовать синхронную репликацию, при которой данные записываются сначала в WAL как минимум одной реплики, после чего исполняются в мастере. При такой схеме необходимо минимум 2 реплики БД, однако вероятность потери данных минимизируется и скорость чтения увеличивается. 
...
* https://www.similarweb.com/ru/website/twitch.tv/
* https://twitchtracker.com/statistics
* https://redis.io/docs/manual/pubsub/
* https://help.twitch.tv/s/article/guide-to-broadcast-health-and-using-twitch-inspector?language=en_US#:~:text=Bitrate%20Spikes,-This%20stream%20may&text=While%20the%20average%20bitrate%20is,to%20buffer%20and%20experience%20issues.
* https://stats.streamelements.com/
* https://reactjs.org/
* https://docs.microsoft.com/ru-ru/exchange/edge-transport-servers
